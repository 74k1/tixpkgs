...
mapfile -t old_output_hashes < <(perl -ne 'print "$1\n" if /^\s*outputHash = "([^"]+)";/' "$package_file")
old_client_hash="${old_output_hashes[0]:-}"
old_server_hash="${old_output_hashes[1]:-}"
...
if [[ "$old_version" == "$new_version" && "$old_src_hash" == "$new_src_hash" ]]; then
  echo "trek source is already at $new_version; refreshing hashes anyway." >&2
fi
...
perl -0pi -e '
  ...
  s/(outputHash = ")\Q$ENV{OLD_CLIENT_HASH}\E(";)/$1$ENV{FAKE_HASH}$2/ or die "failed to replace client outputHash\n";
  s/(outputHash = ")\Q$ENV{OLD_SERVER_HASH}\E(";)/$1$ENV{FAKE_HASH}$2/ or die "failed to replace server outputHash\n";
' "$package_file"
...
new_client_hash="$(get_mismatch_hash 'trek.passthru.trek-client')"
FAKE_HASH="$fake_hash" NEW_CLIENT_HASH="$new_client_hash" perl -0pi -e '
  s/(outputHash = ")\Q$ENV{FAKE_HASH}\E(";)/$1$ENV{NEW_CLIENT_HASH}$2/ or die "failed to replace client hash\n";
' "$package_file"

new_server_hash="$(get_mismatch_hash 'trek.passthru.trek-server-modules')"
FAKE_HASH="$fake_hash" NEW_SERVER_HASH="$new_server_hash" perl -0pi -e '
  s/(outputHash = ")\Q$ENV{FAKE_HASH}\E(";)/$1$ENV{NEW_SERVER_HASH}$2/ or die "failed to replace server hash\n";
' "$package_file"
