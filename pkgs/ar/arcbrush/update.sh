...
package_file="$root/pkgs/ar/arcbrush/default.nix"
api="https://api.arcbrush.com/api/version?platform=linux"

release="$(curl -fsSL "$api")"
new_version="$(jq -r '.version // empty' <<< "$release")"
url="$(jq -r '.download_url // empty' <<< "$release")"

if [[ -z "$new_version" || -z "$url" ]]; then
  echo "failed to read ArcBrush version/download_url from $api" >&2
  exit 1
fi
...
download="$tmpdir/ArcBrush-$new_version-x86_64.AppImage"
...
if [[ "$old_version" == "$new_version" && "$old_hash" == "$new_hash" ]]; then
  echo "arcbrush is already up to date ($new_version)." >&2
  printf '[]\n'
  exit 0
fi
...
printf '[{"attrPath":"arcbrush","oldVersion":"%s","newVersion":"%s","files":["%s"]}]\n' \
