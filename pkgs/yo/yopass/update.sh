...
old_src_hash="$(perl -ne 'print $1 if /^\s*hash = "([^"]+)";/' | head -1 "$pkg_file")"
old_vendor_hash="$(perl -ne 'print $1 if /^\s*vendorHash = "([^"]+)";/' "$pkg_file")"
old_yarn_hash="$(perl -ne 'print $1 if /^\s*hash = "([^"]+)";/' | tail -1 "$pkg_file")"
...
# 2. Replace yarn lock hash (last hash in the file) with fake, build, extract real one
perl -0pi -e '
