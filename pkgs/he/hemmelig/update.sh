...
package_file="$root/pkgs/he/hemmelig/default.nix"
...
if [[ -z "$old_version" || -z "$old_src_hash" || -z "$old_npm_hash" || -z "$new_version" ]]; then
  echo "failed to read current or latest hemmelig version/hashes" >&2
  exit 1
fi

if [[ "$old_version" == "$new_version" ]]; then
  echo "hemmelig is already at $new_version; refreshing hashes anyway." >&2
fi
...
log="$(mktemp)"
if nix build --no-link --print-build-logs "$root#hemmelig" >"$log" 2>&1; then
...
printf '[{"attrPath":"hemmelig","oldVersion":"%s","newVersion":"%s","files":["%s"]}]\n' \
