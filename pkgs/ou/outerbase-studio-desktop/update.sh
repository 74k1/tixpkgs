...
package_file="$root/pkgs/ou/outerbase-studio-desktop/default.nix"
repo="outerbase/studio-desktop"
...
if [[ -z "$old_version" || -z "$old_hash" || -z "$new_version" ]]; then
  echo "failed to read current or latest outerbase-studio-desktop version/hash" >&2
  exit 1
fi

url="https://github.com/$repo/releases/download/v${new_version}/Outerbase-Studio-Linux-${new_version}.AppImage"

new_hash="$(nix --extra-experimental-features nix-command store prefetch-file --json "$url" | jq -r '.hash')"

if [[ -z "$new_hash" ]]; then
  echo "failed to prefetch outerbase-studio-desktop $new_version" >&2
  exit 1
fi

if [[ "$old_version" == "$new_version" && "$old_hash" == "$new_hash" ]]; then
  echo "outerbase-studio-desktop is already up to date ($new_version)." >&2
  printf '[]\n'
  exit 0
fi
...
printf '[{"attrPath":"outerbase-studio-desktop","oldVersion":"%s","newVersion":"%s","files":["%s"]}]\n' \
